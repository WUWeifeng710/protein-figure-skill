# ss_only_surface template render script reference (2.5.0 strict API, validated with 1AY7 sample figure)
# Pure cartoon pastels by secondary structure (ss h=soft_blue / ss s=rose_pink / loop=lavender)
# No surface, no object split; color m directly
# Run: execute in an environment with PyMOL 2.5.0 + PyMOL-PUB installed, e.g. `conda run -n pymol_pub python ss_only_surface.py`
import os
...
# 4) cartoon colored by secondary structure directly on m (no object split, no surface)
